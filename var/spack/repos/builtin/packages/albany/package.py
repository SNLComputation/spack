...
class Albany(CMakePackage):
    ...
    # Add dependencies
    depends_on('mpi')
    depends_on('trilinos@master,develop~superlu-dist+exodus+chaco+isorropia+tempus+rythmos+teko+intrepid+intrepid2+minitensor+phalanx+pnetcdf+nox+piro+rol+shards+stk+amesos2~metis~hypre+ifpack2~mumps~matio~suite-sparse gotype=long_long')
